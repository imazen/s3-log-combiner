...
pub(crate) static TIME_SPENT_WRITING: AtomicU64 = AtomicU64::new(0);
...
pub(crate) async fn print_status_update_tokio(config: FetchArgs) {
    ...
    loop {
        print_status_update(false, &config);
        sleep(Duration::from_secs(UPDATE_FREQUENCY_SECONDS as u64)).await;
    }
}
pub(crate) fn print_status_update(force: bool, config: &FetchArgs) {
    ...
    if force || now - last_update >= UPDATE_FREQUENCY_SECONDS as u64 {
        if last_update != 0 {
            ...
            let avg_blobs_per_list = FETCHES_ENQUEUED.load(std::sync::atomic::Ordering::Relaxed)
            ...
            let total_ms_writing = TIME_SPENT_WRITING.load(std::sync::atomic::Ordering::Relaxed);

            println!("{:.2}Mbps down, {:.2}Mbps disk, {} fetches queued, {} results queued, {} fatal errors, {:.2} Gb written/compressed in {}ms, {:.2} Gb fetched, {} HTTP requests, {} avg blobs/listing",
                     mbps_read, mbps_written, fetches_net, blob_results_net, total_unrecoverable_errors, total_gb_written, total_ms_writing, total_gb_read, total_requests, avg_blobs_per_list);
        }
        ...
        if last_update == 0 {
            print_status_update(force, &config);
        }
    }
}
